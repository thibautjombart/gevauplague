...
## Given the reported ESS a thining of 1/25 seems reasonable
to_keep <- seq(from = 1, to = n_iter, by = 25)
...
- the average value of estimated $R_0$ is `r round(chains_smry[1, 1], 2)` (95% CrI: `r round(chains_smry[1, 3], 2)` - `r round(chains_smry[1, 4], 2)`), with `r 100 * round(p_R_above_1, 3)`% of posterior values above 1, suggesting that person-to-person transmission only is unlikely to have sustained the epidemic
...
- the average value of estimated $R_0$ using all cases is `r round(chains_smry[1, 1], 2)` (95% CrI: `r round(chains_smry[1, 3], 2)` - `r round(chains_smry[1, 4], 2)`), with `r 100 * round(p_R_above_1, 3)`% of posterior values above 1
