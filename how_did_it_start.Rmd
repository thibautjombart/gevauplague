...
3. propose (using symmetric proposal distributions) new values for $\theta^*$
   ...
   movements are used for $\lambda_z$ and $R$, using Normal proposal distributions
...
Here the estimates are based on 10 million draws:
```{r}
## RNG for delay from onset to infection
onset_to_inf_r <- function(n) {
  floor(runif(n, 0, infec$r(n) + 1))
}

## PMF for delays of 0, 1, ...
gentime_dat <- incub$r(1e7) + onset_to_inf_r(1e7)
range(gentime_dat)
gentime_freq <- sapply(
  seq(0, max(gentime_dat), by = 1L), 
  function(i) sum(i == gentime_dat)
)
gentime_pmf <- gentime_freq
...
gentime_r <- function(n) {
  sample(gentime_dat, size = n, replace = TRUE)
}
...
system.time(res <- estimate_params(n_iter = 10))
head(res)
tail(res)
...
##   cbind.data.frame(estimate_params(n_iter = 1000), chain = i),
##   mc.cores = 12
##   )
## saveRDS(res, file = "res_1000iter_12chains_all_data.rds")

res <- readRDS("res_1000iter_12chains_all_data.rds")
...
to_keep <- seq(from = 1, to = 1000, by = 10)
...
- the data is informative on $R$ and $\lambda_z$, with posterior distributions well different from the priors
- $R$ values were most likely below 1, with a mean of `r round(chains_smry[1, 1], 2)` (95% CrI: `r round(chains_smry[1, 3], 2)` - `r round(chains_smry[1, 4], 2)`)
- zoonotic introductions played a substantial role in transmission, with a mean rate of introduction of `r round(chains_smry[2, 1], 2)` (95% CrI: `r round(chains_smry[2, 3], 2)` - `r round(chains_smry[2, 4], 2)`), corresponding to an average of one introduction every `r round(1/chains_smry[2, 1], 1)` days
