...
```{r}
#prior_zoo <- function(x, log = FALSE) dexp(x, 0.2, log = log)
prior_zoo <- function(x, log = FALSE) dunif(x, 0, 10, log = log)
plot(prior_zoo, xlim = c(0, 5), main = "Prior for lambda_z")
```
...
We run the chains for a few iterations:
...
We can try to run several chains in parallel:

```{r}

# library(future.apply)
# plan(multisession, workers = 8)
# res <- future_lapply(1:8, function(i) estimate_params(n_iter = 100))

#res <- lapply(1:8, function(i) 
#  cbind.data.frame(estimate_params(n_iter = 100), chain = i)
#  )

res <- readRDS("res_1000iter_8chains_04-10-2024.rds")

#plan(sequential)

chains <- Reduce(rbind, lapply(1:length(res), function(i) res[[i]]))
...
chains %>% 
  ...
  geom_density() + 
  geom_rug() +
  labs(title = "Posterior distribution of R")
