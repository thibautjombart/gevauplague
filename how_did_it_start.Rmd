...
where $f(.)$ is the distribution of the infectious period duration.
...
make_aug_data <- function(
    deaths = x$date_of_death, 
    f = infec$r) {
    deaths - f(length(deaths))
}

## Function to calculate the log-likelihood
compute_loglike <- function(x, params, w = si$d(0:50)) {
  ### turn data into time since first case, exclude 1st one
  y <- as.integer(x - min(x))
  last_day <- max(y)
  
  ## calculate incidence for the whole time period, do no miss the zeros
  incid <- sapply(seq(0, last_day, by = 1), function(i) sum(y == i))
 
  ## get relative FOIs
  lambdas_p2p <- EpiEstim::overall_infectivity(incid, w)
  lambdas <- params$zoo + lambdas_p2p * params$R
  out <- dpois(incid, lambdas, log = TRUE)
  sum(na.omit(out))
}
...
compute_priors <- function(params) {
  prior_R(params$R, log = TRUE) + prior_zoo(params$zoo, log = TRUE)
}

## Compute log-posterior
compute_post <- function(x, params, w = si$d(0:50)) {
  compute_loglike(x, params, w) + compute_priors(params)
}
